...
import { TranscriptionResult } from 'react-native-executorch/legacy';

export const VerboseTranscription = ({ data }: { data: TranscriptionResult }) => {
  if (!data) return null;

  const hasSegments = Array.isArray(data.segments) && data.segments.length > 0;

  const hasLanguage = !!data.language && data.language !== 'N/A' && data.language.trim() !== '';
  ...
  return (
    <View style={styles.container}>
      <View style={styles.metaContainer}>
        ...
        {hasMetadata && (
          <View style={styles.row}>
            {hasLanguage && <Text style={styles.metaItem}>Language: {data.language}</Text>}
            {hasDuration && (
              <Text style={styles.metaItem}>Duration: {data.duration?.toFixed(2)}s</Text>
            )}
          </View>
        )}
      </View>
      ...
      {hasSegments && (
        <>
          <Text style={styles.sectionHeader}>Segments ({data.segments?.length})</Text>
              ...
              <View style={styles.statsGrid}>
                <View style={styles.statItem}>
                  ...
                  <Text style={styles.statValue}>
                    {data.task === 'transcribe' ? seg.avgLogprob?.toFixed(4) : 'N/A'}
                  </Text>
                </View>
                <View style={styles.statItem}>
                  <Text style={styles.statLabel}>Temp</Text>
                  <Text style={styles.statValue}>
                    {data.task === 'transcribe' ? seg.temperature?.toFixed(2) : 'N/A'}
                  </Text>
                </View>
                <View style={styles.statItem}>
                  ...
                  <Text style={styles.statValue}>
                    {data.task === 'transcribe' ? seg.compressionRatio?.toFixed(2) : 'N/A'}
                  </Text>
                </View>
              </View>
            </View>

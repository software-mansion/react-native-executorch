...
import { models, useLLM, useTextToSpeech } from 'react-native-executorch/legacy';
import {
  AudioManager,
  AudioContext,
  AudioBuffer,
...
export const TextToSpeechLLMScreen = ({ onBack }: TextToSpeechLLMProps) => {
  ...
  const getModelStatus = () => {
    ...
    if (!llm.isReady) return `Loading LLM: ${(100 * llm.downloadProgress).toFixed(2)}%`;
    if (!tts.isReady) return `Loading TTS: ${(100 * tts.downloadProgress).toFixed(2)}%`;

...
import { models, useVAD } from 'react-native-executorch/legacy';
...
export const VoiceActivityDetectionScreen = ({ onBack }: { onBack: () => void }) => {
  const model = useVAD({
  ...
  const recordingButtonDisabled = isSimulator || !readyToStream || model.isGenerating;
      ...
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.visualizerContainer}>
          <FontAwesome
            name="volume-up"
            size={100}
            color={isSpeaking ? styles.speakingText.color : styles.silentText.color}
          />
          <Text
          ...
          {isStreaming ? (
            ...
            >
              ...
              <Text style={styles.buttonText}>
                {isSimulator ? 'Recording not available on Simulator' : 'Start VAD Stream'}
              </Text>
            </TouchableOpacity>
          )}
        </View>

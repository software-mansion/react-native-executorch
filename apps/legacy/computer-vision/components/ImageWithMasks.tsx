...
export function buildDisplayInstances(
  ...
  return rawInstances
    .map((inst, i) => {
      ...
      const img = createMaskImage(inst.mask, inst.maskWidth, inst.maskHeight, color);
      if (!img) return null;
      return {
        bbox: inst.bbox,
...
export default function ImageWithMasks({ imageUri, instances, imageWidth, imageHeight }: Props) {

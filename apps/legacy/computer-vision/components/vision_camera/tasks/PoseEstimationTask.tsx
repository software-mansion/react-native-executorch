...
import { models, usePoseEstimation, PoseDetections } from 'react-native-executorch/legacy';
...
export default function PoseEstimationTask({
  ...
  const updateDetections = useCallback(
    (p: { results: PoseDetections; imageWidth: number; imageHeight: number }) => {
      ...
      <Svg style={StyleSheet.absoluteFill}>
        {detections.map((personKeypoints, personIdx) => {
          ...
          const isVisible = (kp: { x: number; y: number }) =>
            kp.x >= 0 && kp.y >= 0 && kp.x <= imageSize.width && kp.y <= imageSize.height;
          return (

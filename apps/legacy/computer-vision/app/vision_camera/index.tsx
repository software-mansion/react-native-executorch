...
export default function VisionCameraScreen() {
  ...
  const [cameraPositionSync] = useState(() => createSynchronizable<'front' | 'back'>('back'));
  ...
  const [frameOutput, setFrameOutput] = useState<ReturnType<typeof useFrameOutput> | null>(null);
  ...
  return (
    <View style={styles.container}>
      ...
      <Camera
        ...
        orientationSource={activeTask === 'segmentation' ? 'interface' : 'device'}
        onError={(e) => {
      ...
      {activeTask === 'poseEstimation' && (
        <PoseEstimationTask {...taskProps} activeModel={activeModel as 'poseEstimationYolo26n'} />
      )}
      {activeTask === 'ocr' && <OCRTask {...taskProps} />}
      {activeTask === 'styleTransfer' && (
        <StyleTransferTask
          {...taskProps}
          activeModel={activeModel as 'styleTransferCandy' | 'styleTransferMosaic'}
        />
      )}
      ...
      <View style={[styles.topOverlay, { paddingTop: insets.top + 8 }]} pointerEvents="box-none">
        ...
        >
          {TASKS.map((t) => (
            ...
            >
              <Text style={[styles.tabText, activeTask === t.id && styles.tabTextActive]}>
                {t.label}
              </Text>
            </TouchableOpacity>
          ))}
        ...
        >
          {activeTaskInfo.variants.map((v) => (
            <TouchableOpacity
              key={v.id}
              style={[styles.variantChip, activeModel === v.id && styles.variantChipActive]}

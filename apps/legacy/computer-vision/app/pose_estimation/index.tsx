...
export default function PoseEstimationScreen() {
  ...
  const [selectedModel, setSelectedModel] = useState<PoseEstimationModelSources>(
    poseEstimation.yolo26n()
  );
              ...
              {results.length > 0 &&
                ...
                (() => {
                  ...
                  if (imageRatio > layoutRatio) {
                    ...
                    scaleX = (layout.height * imageRatio) / imageDimensions.width;
                  }
                  const offsetX = (layout.width - imageDimensions.width * scaleX) / 2;
                  const offsetY = (layout.height - imageDimensions.height * scaleY) / 2;
                        ...
                        return (
                          <React.Fragment key={`person-${personIdx}`}>
                            {COCO_SKELETON_CONNECTIONS.map(([from, to], lineIdx) => {
                              ...
                              if (!isInBounds(kp1) || !isInBounds(kp2)) return null;
                              return (
                                <Line
        ...
        {!imageUri && (
          <View style={styles.infoContainer}>
            ...
            <Text style={styles.infoText}>
              This model detects human body keypoints (17 COCO keypoints) and draws a skeleton
              overlay. Pick an image from your gallery or take one with your camera to get started.
            </Text>
          </View>
        )}
      </View>

...
export default function SegmentAnythingScreen() {
  ...
  const [selectedModel, setSelectedModel] = useState<InstanceSegmentationModelSources>(
  ...
  const { isReady, isGenerating, downloadProgress, forward, error } = useInstanceSegmentation({
    model: selectedModel,
  });
  ...
  function applyMatch(match: SegmentedInstance<typeof FastSAMLabel> | null): void {
  ...
  function handleTap(e: GestureResponderEvent) {
    ...
    const c = touchToImageCoords(e.nativeEvent.locationX, e.nativeEvent.locationY);
    if (!c) return;
    applyMatch(selectByPoint(rawInstancesRef.current, Math.round(c.x), Math.round(c.y)));
  }
  ...
  async function runTextPrompt() {
    ...
    try {
      ...
      const match = selectByText(instances, instanceEmbeddingsRef.current, textEmb);
      applyMatch(match);
    } catch (e) {
      console.error(e);
    } finally {
  ...
  if (!isReady) {
    return (
      <Spinner visible textContent={`Loading the model ${(downloadProgress * 100).toFixed(0)} %`} />
    );
  }
        ...
        >
              ...
              {!imageUri && (
                <View style={styles.infoContainer}>
                  ...
                  <Text style={styles.infoText}>
                    Segment any object in an image. (1) Pick an image, (2) tap Run to detect
                    instances, (3) tap a point, draw a box, or describe an object to segment it.
                  </Text>
                </View>
              )}
            </View>
          ...
          {mode === 'text' && embeddingProgress && (
            <Text style={styles.statusLine}>
              Embedding instances {embeddingProgress.done}/{embeddingProgress.total} (subsequent
              text queries are instant)
            </Text>
          )}
          ...
          <StatsBar
            ...
            detectionCount={
              rawInstancesRef.current.length > 0 ? rawInstancesRef.current.length : null
            }
          />

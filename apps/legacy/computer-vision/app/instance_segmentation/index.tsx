...
export default function InstanceSegmentationScreen() {
  const [selectedModel, setSelectedModel] = useState<InstanceSegmentationModelSources>(
    instanceSegmentation.yolo26n()
  );
  const [inferenceTime, setInferenceTime] = useState<number | null>(null);

  const { setGlobalGenerating } = useContext(GeneratingContext);

  const { isReady, isGenerating, downloadProgress, forward, error, getAvailableInputSizes } =
    useInstanceSegmentation({
      model: selectedModel,
    });
  ...
  const [selectedInputSize, setSelectedInputSize] = useState<number | null>(null);
      ...
      <View style={styles.container}>
          ...
          {!imageUri && (
            <View style={styles.infoContainer}>
              ...
              <Text style={styles.infoText}>
                This model detects individual objects and draws a precise mask over each one. Pick
                an image from your gallery or take one with your camera to get started.
              </Text>
            </View>
          )}
        </View>
          ...
          <View style={styles.inputSizeContainer}>
            ...
            >
              {availableInputSizes.map((size) => (
                <TouchableOpacity
                  key={size}
                  style={[styles.sizeButton, selectedInputSize === size && styles.sizeButtonActive]}
        ...
        {instances.length > 0 && (
          <View style={styles.resultsContainer}>
            <Text style={styles.resultsHeader}>Detected {instances.length} instance(s)</Text>
            <ScrollView style={styles.resultsList}>
              {instances.map((instance, idx) => (
                <View key={idx} style={styles.resultRow}>
                  <Text style={styles.resultText}>
                    {instance.label || 'Unknown'} ({(instance.score * 100).toFixed(1)}%)
                  </Text>
                </View>
              ))}
            </ScrollView>

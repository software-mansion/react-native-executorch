...
function ClipEmbeddingsScreen() {
  const [selectedImageModel, setSelectedImageModel] = useState<ImageEmbeddingModel>(
  ...
  const [results, setResults] = useState<{ label: string; similarity: number }[]>([]);
  const [imageEmbeddingTime, setImageEmbeddingTime] = useState<number | null>(null);
  const [textEmbeddingTime, setTextEmbeddingTime] = useState<number | null>(null);

  const getModelStatusText = (model: typeof textModel | typeof imageModel) => {
    if (model.error) return `Oops! ${model.error}`;
    if (!model.isReady) return `Loading ${(model.downloadProgress * 100).toFixed(0)}%`;
      ...
      >
        ...
        >
          ...
          <View style={styles.statusRow}>
            <Text style={styles.statusText}>Text model: {getModelStatusText(textModel)}</Text>
            <Text style={styles.statusText}>Image model: {getModelStatusText(imageModel)}</Text>
          </View>
          ...
          {/* Classify button */}
          <TouchableOpacity
            style={[styles.classifyButton, (!imageUri || !modelsReady) && styles.buttonDisabled]}
            onPress={classify}
            ...
            <View style={styles.card}>
              ...
              {results.map((item, index) => (
                <View key={item.label} style={styles.resultRow}>
                  <Text style={[styles.resultLabel, index === 0 && styles.topResultLabel]}>
                    {index === 0 ? '🥇 ' : ''}
                    {item.label}
                  </Text>
                  <Text style={styles.resultScore}>{item.similarity.toFixed(3)}</Text>
                </View>
              ))}
              {(imageEmbeddingTime !== null || textEmbeddingTime !== null) && (
                <View style={styles.statsContainer}>
                  {imageEmbeddingTime !== null && (
                    <Text style={styles.statsText}>Image embedding: {imageEmbeddingTime} ms</Text>
                  )}
                  {textEmbeddingTime !== null && (
                    <Text style={styles.statsText}>Text embeddings: {textEmbeddingTime} ms</Text>
                  )}
                </View>
              )}
            </View>
          ...
          <View style={styles.card}>
            ...
            <View style={styles.addLabelRow}>
              ...
              />
              <TouchableOpacity
                style={[styles.addButton, !newLabel.trim() && styles.buttonDisabled]}
                onPress={addLabel}
                disabled={!newLabel.trim()}
              >
                <Ionicons name="add" size={20} color={newLabel.trim() ? 'white' : 'gray'} />
              </TouchableOpacity>
            </View>
          </View>
        </ScrollView>

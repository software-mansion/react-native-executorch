...
import { buildSegments, colorForLabel, matchEntities } from '../../utils/piiMatching';
...
function sampleFor(model: PrivacyFilterModelSources): string {
  return model.modelName === privacyFilter.nemotron().modelName ? NEMOTRON_SAMPLE : OPENAI_SAMPLE;
}

function HighlightedText({ source, entities }: { source: string; entities: PiiEntity[] }) {
  ...
  return (
    <Text style={styles.sampleText}>
      {segments.map((seg, i) =>
        seg.label ? (
          <Text key={i} style={[styles.highlight, { backgroundColor: colorForLabel(seg.label) }]}>
            {seg.text}
          </Text>
        ) : (
    ...
    <View style={[styles.container, { paddingBottom: bottom + 8 }]}>
      ...
      {filter.error && (
        <View style={styles.errorBanner}>
          <Text style={styles.errorText}>Load error: {filter.error.message}</Text>
        </View>
      )}
      ...
      {!filter.isReady && !filter.error && (
        <View style={styles.centerBlock}>
          ...
          <Text style={styles.muted}>
            Downloading model… {Math.round((filter.downloadProgress ?? 0) * 100)}%
          </Text>
        </View>
      )}

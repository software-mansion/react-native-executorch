...
import { models, useLLM, DEFAULT_SYSTEM_PROMPT } from 'react-native-executorch/legacy';
...
function LLMToolCallingScreen() {
  ...
  const [selectedModel, setSelectedModel] = useState<LLMModelSources>(models.llm.hammer2_1_1_5b());
  const textInputRef = useRef<TextInput>(null);
  const { setGlobalGenerating } = useContext(GeneratingContext);

  const llm = useLLM({ model: selectedModel });
  const tokenCount = llm.isReady ? llm.getGeneratedTokenCount() : 0;
  const { stats, onMessageSend } = useLLMStats(llm.response, llm.isGenerating, tokenCount);
  ...
  const requestBrightnessPermission = async () => {
    ...
    if (status === Brightness.PermissionStatus.UNDETERMINED || canAskAgain) {
      ...
      setHasBrightnessPermission(nextStatus === Brightness.PermissionStatus.GRANTED);
      return;
    }
        ...
        >
          ...
          {llm.messageHistory.length ? (
            ...
            <View style={styles.helloMessageContainer}>
              ...
              <SuggestedPrompts prompts={SUGGESTED_PROMPTS} onSelect={setUserInput} />
            </View>
          )}

          {!hasCalendarPermission && (
            <TouchableOpacity style={styles.permissionBanner} onPress={requestCalendarPermission}>
          ...
          {!hasBrightnessPermission && (
            <TouchableOpacity style={styles.permissionBanner} onPress={requestBrightnessPermission}>
              <Text style={styles.permissionBannerText}>
          ...
          <View style={styles.bottomContainer}>
            ...
            {llm.isGenerating && (
              <TouchableOpacity style={styles.sendChatTouchable} onPress={llm.interrupt}>

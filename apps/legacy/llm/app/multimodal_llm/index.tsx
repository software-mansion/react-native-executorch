...
function MultimodalLLMScreen() {
  ...
  const { stats, onMessageSend } = useLLMStats(vlm.response, vlm.isGenerating, tokenCount);
    ...
    <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
      ...
      >
        <View style={styles.container}>
          ...
          {imageUri && (
            <TouchableOpacity style={styles.imageThumbnailContainer} onPress={pickImage}>
          ...
          <View style={styles.audioUrlRow}>
            ...
            <TouchableOpacity
              style={[
                ...
                (!audioUrl.trim() || isFetchingAudio || vlm.isGenerating) && styles.disabled,
              ]}
              onPress={loadAudioFromUrl}
              disabled={!audioUrl.trim() || isFetchingAudio || vlm.isGenerating}
            >
              <Text style={styles.audioUrlButtonText}>{isFetchingAudio ? '…' : 'Load'}</Text>
            </TouchableOpacity>
          </View>
          ...
          {audioLabel && (
            <View style={styles.audioAttachmentContainer}>
              ...
              <AudioWaveform buffer={audioBuffer} style={styles.audioWaveform} />
            </View>
          )}
          ...
          >
            ...
            {(imageUri || audioBuffer || userInput.trim()) && !vlm.isGenerating && (
              <TouchableOpacity style={styles.sendChatTouchable} onPress={sendMessage}>
                <SendIcon height={24} width={24} padding={4} margin={8} />
              </TouchableOpacity>
            )}
            {vlm.isGenerating && (
              <TouchableOpacity style={styles.sendChatTouchable} onPress={vlm.interrupt}>

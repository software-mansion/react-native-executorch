...
export function useLLMStats(response: string, isGenerating: boolean, totalTokens: number) {
  ...
  useEffect(() => {
    if (!isGenerating && sendTimeRef.current !== null && firstTokenTimeRef.current !== null) {
      const endTime = Date.now();
      const ttft = firstTokenTimeRef.current - sendTimeRef.current;
      const totalTime = (endTime - firstTokenTimeRef.current) / 1000;
      const tokensPerSec = totalTime > 0 ? Math.round(totalTokens / totalTime) : 0;

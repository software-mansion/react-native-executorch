...
import { ToolCall } from 'react-native-executorch/legacy';

export const executeTool: (call: ToolCall) => Promise<string | null> = async (call) => {
...
const brightness = async (call: ToolCall) => {
  ...
  if ('targetBrightness' in call.arguments && typeof call.arguments.targetBrightness === 'number') {
    ...
    await Brightness.setBrightnessAsync(
      clamp((await Brightness.getBrightnessAsync()) + call.arguments.relativeChange / 100, 0, 1)
    );
  }
  return null;
};
...
const readCalendar = async (call: ToolCall) => {
  ...
  let startTime = Date.parse(
    'timeStart' in call.arguments && typeof call.arguments.timeStart === 'string'
  ...
  if (startTime === endTime || (Number.isNaN(startTime) && Number.isNaN(endTime))) {
    ...
    startTime = new Date(date.getFullYear(), date.getMonth(), date.getDate(), 0, 0, 0).valueOf();

    // Set the time to 23:59:59 for the end of the day
    endTime = new Date(date.getFullYear(), date.getMonth(), date.getDate(), 23, 59, 59).valueOf();

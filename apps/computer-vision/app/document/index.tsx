...
import { useOcr, models, type DocumentBlock } from 'react-native-executorch';
...
// cached on-device from Hugging Face by `useOcr`. Backends are filtered
...
function DocumentContent() {
  ...
  // Hosted configs — `useOcr` downloads + caches each enabled model.
  // orientation/dewarp are NOT baked here: they're passed per-run to
  // `runOcr` below, so toggling them takes effect without a reload.
  const config = {
    ...models.ocr.PADDLE.PPOCRV6_SMALL[backend.key],
  ...
  const run = async () => {
    ...
    try {
      ...
      if (!frameImage) throw new Error('Failed to build the processed frame image');

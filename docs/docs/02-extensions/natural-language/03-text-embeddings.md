...
This enables on-device semantic search, offline Retrieval-Augmented Generation (RAG) against local SQLite vector stores, intent classification, and cross-modal text-to-image queries when paired with [Image Embeddings](../computer-vision/10-image-embeddings.md) — entirely on the client without sending private text to cloud APIs.

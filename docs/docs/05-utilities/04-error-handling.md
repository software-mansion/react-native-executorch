...
| `ResourceFetcherFileSystemApiUnavailable` | Expo FileSystem API is unavailable (occurs for ExpoResourceFetcher) | When importing a non-existing Expo FS module                         | If you're using Expo 54 and below, make sure to use `/legacy` import for ExpoResourceFetcher. |

/**
 * Creating a sidebar enables you to:
 - create an ordered group of docs
 - render a sidebar for each doc of that group
 - provide next/previous navigation

 The sidebars can be generated from the filesystem, or explicitly defined here.

 Create as many sidebars as you want.
 */

// @ts-check

/** @type {import('@docusaurus/plugin-content-docs').SidebarsConfig} */

const sidebars = {
  // By default, Docusaurus generates a sidebar from the docs folder structure
  tutorialSidebar: [
    {
      type: 'category',
      label: 'Fundamentals',
      collapsed: false,
      items: ['fundamentals/getting-started', 'fundamentals/loading-models'],
    },
    {
      type: 'category',
      label: 'Hooks',
      collapsed: false,
      items: [
        {
          type: 'category',
          label: 'Natural Language Processing',
          link: {
            type: 'generated-index',
          },
          items: [
            'llms/useLLM',
            'speech-to-text/useSpeechToText',
          ],
        },
        {
          type: 'category',
          label: 'Computer Vision',
          link: {
            type: 'generated-index',
          },
          items: [
            'computer-vision/useClassification',
            'computer-vision/useImageSegmentation',
            'computer-vision/useOCR',
            'computer-vision/useObjectDetection',
            'computer-vision/useStyleTransfer',
            'computer-vision/useVerticalOCR',
          ],
        },
      ],
    },
        {
      type: 'category',
      label: 'Typescript API',
      collapsed: false,
      items: [
        {
          type: 'category',
          label: 'Natural Language Processing',
          link: {
            type: 'generated-index',
          },
          items: [
            'hookless-api/LLMModule',
            'hookless-api/SpeechToTextModule',
          ],
        },
        {
          type: 'category',
          label: 'Computer Vision',
          link: {
            type: 'generated-index',
          },
          items: [
            'hookless-api/ClassificationModule',
            'hookless-api/ImageSegmentationModule',
            'hookless-api/OCRModule',
            'hookless-api/ObjectDetectionModule',
            'hookless-api/StyleTransferModule',
            'hookless-api/VerticalOCRModule',
          ],
        },
      ],
    }
  ]
}
// tutorialSidebar: [{ type: 'autogenerated', dirName: '.' }],
//   tutorialSidebar: [
//     {
//       type: 'category',
//       label: 'Fundamentals',
//       collapsed: false,
//       items: ['fundamentals/getting-started', 'fundamentals/loading-models'],
//     },
//     {
//       type: 'category',
//       label: 'Hooks',
//       link: {
//         type: 'generated-index',
//       },
//       collapsed: false,
//       items: [
//         {
//           type: 'category',
//           label: 'Natural Language Processing',
//           link: {
//             type: 'generated-index',
//           },
//           items: [
//             'natural-language-processing/useLLM',
//             'natural-language-processing/useSpeechToText',
//             'natural-language-processing/useTextEmbeddings',
//           ],
//         },
//         {
//           type: 'category',
//           label: 'Computer Vision',
//           link: {
//             type: 'generated-index',
//           },
//           items: [
//             'computer-vision/useClassification',
//             'computer-vision/useImageSegmentation',
//             'computer-vision/useOCR',
//             'computer-vision/useObjectDetection',
//             'computer-vision/useStyleTransfer',
//             'computer-vision/useVerticalOCR',
//           ],
//         },
//         {
//           type: 'category',
//           label: 'Executorch Bindings',
//           link: {
//             type: 'generated-index',
//           },
//           items: ['executorch-bindings/useExecutorchModule'],
//         },
//       ],
//     },
//     {
//       type: 'category',
//       label: 'Typescript API',
//       link: {
//         type: 'generated-index',
//       },
//       collapsed: false,
//       items: [
//         {
//           type: 'category',
//           label: 'Natural Language Processing',
//           link: {
//             type: 'generated-index',
//           },
//           items: [
//             'typescript-api/LLMModule',
//             'typescript-api/SpeechToTextModule',
//             'typescript-api/TextEmbeddingsModule',
//           ],
//         },
//         {
//           type: 'category',
//           label: 'Computer Vision',
//           link: {
//             type: 'generated-index',
//           },
//           items: [
//             'typescript-api/ClassificationModule',
//             'typescript-api/ImageSegmentationModule',
//             'typescript-api/OCRModule',
//             'typescript-api/ObjectDetectionModule',
//             'typescript-api/StyleTransferModule',
//             'typescript-api/VerticalOCRModule',
//           ],
//         },
//         {
//           type: 'category',
//           label: 'Executorch Bindings',
//           link: {
//             type: 'generated-index',
//           },
//           items: ['typescript-api/ExecutorchModule'],
//         },
//       ],
//     },
//     {
//       type: 'category',
//       label: 'Benchmarks',
//       link: {
//         type: 'generated-index',
//       },
//       collapsed: false,
//       items: [
//         'benchmarks/inference-time',
//         'benchmarks/memory-usage',
//         'benchmarks/model-size',
//       ],
//     },

//     // { type: 'autogenerated', dirName: '.' }
//   ],
//   // faq/frequently-asked-questions
//   // fundamentals/getting-started
//   // fundamentals/loading-models
//   // utilities/list-downloaded-resources
//   // But you can create a sidebar manually
//   /*
//   tutorialSidebar: [
//     'intro',
//     'hello',
//     {
//       type: 'category',
//       label: 'Tutorial',
//       items: ['tutorial-basics/create-a-document'],
//     },
//   ],
//    */

module.exports = sidebars;
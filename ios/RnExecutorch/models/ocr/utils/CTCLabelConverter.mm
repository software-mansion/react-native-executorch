...
- (instancetype)initWithCharacters:(NSString *)characters
  ...
  if (self) {
    ...
    NSMutableArray *mutableCharacters =
        [NSMutableArray arrayWithObject:@"[blank]"];

    for (NSUInteger i = 0; i < [characters length]; i++) {
      NSString *charStr =
          [NSString stringWithFormat:@"%C", [characters characterAtIndex:i]];
...
- (NSArray<NSString *> *)decodeGreedy:(NSArray<NSNumber *> *)textIndex
  ...
  while (index < textIndex.count) {
    ...
    NSMutableArray<NSNumber *> *isNotRepeated =
        [NSMutableArray arrayWithObject:@YES];

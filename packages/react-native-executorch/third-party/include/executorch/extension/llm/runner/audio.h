...
public:
  ...
  Audio(std::vector<uint8_t> &&data, int32_t batch_size, int32_t n_bins,
        int32_t n_frames)
      : data_(std::move(data)), batch_size_(batch_size), n_bins_(n_bins),
        n_frames_(n_frames) {
    ET_CHECK_MSG(
        data_.index() == 0 &&
  ...
  Audio(std::vector<float> &&data, int32_t batch_size, int32_t n_bins,
        int32_t n_frames)
      : data_(std::move(data)), batch_size_(batch_size), n_bins_(n_bins),
        n_frames_(n_frames) {
    ET_CHECK_MSG(
  ...
  int32_t get_n_bins() const { return n_bins_; }
  int32_t get_n_frames() const { return n_frames_; }
  /**
  ...
  toTensor(bool with_batch = false) const {
    ...
    ET_LOG(Error, "Shouldn't reach here, audio data is not initialized with "
                  "uint8_t or float vector.");

...
public:
  ...
  static inline std::string TypeToString(Type t) { return TypeName(t); }
   ...
   * "unknown".
   */
  const char *type_name() const noexcept { return TypeName(get_type()); }
   ...
   * @throws std::bad_variant_access if this input doesn't contain text.
   */
  std::string &get_text() & { return std::get<std::string>(data_); }
   ...
   * @throws std::bad_variant_access if this input doesn't contain an image.
   */
  Image &get_image() & { return std::get<Image>(data_); }
   ...
   * @throws std::bad_variant_access if this input doesn't contain an image.
   */
  Image &&get_image() && { return std::get<Image>(std::move(data_)); }
   ...
   */
  const Audio &get_audio() const & { return std::get<Audio>(data_); }
   ...
   * @throws std::bad_variant_access if this input doesn't contain audio.
   */
  Audio &get_audio() & { return std::get<Audio>(data_); }
   ...
   * @throws std::bad_variant_access if this input doesn't contain audio.
   */
  Audio &&get_audio() && { return std::get<Audio>(std::move(data_)); }
   ...
   */
  const RawAudio &get_raw_audio() const & { return std::get<RawAudio>(data_); }
   ...
   * @throws std::bad_variant_access if this input doesn't contain raw audio.
   */
  RawAudio &get_raw_audio() & { return std::get<RawAudio>(data_); }
   ...
   * @throws std::bad_variant_access if this input doesn't contain raw audio.
   */
  RawAudio &&get_raw_audio() && { return std::get<RawAudio>(std::move(data_)); }
   ...
   * nullptr otherwise.
   */
  Image *try_get_image() noexcept { return std::get_if<Image>(&data_); }
   ...
   * nullptr otherwise.
   */
  Audio *try_get_audio() noexcept { return std::get_if<Audio>(&data_); }
...
inline MultimodalInput
make_token_input(const std::vector<uint64_t> &tokens) noexcept {
  return MultimodalInput(tokens);
}

inline MultimodalInput
make_token_input(std::vector<uint64_t> &&tokens) noexcept {
...
inline MultimodalInput
make_raw_audio_input(const RawAudio &raw_audio) noexcept {

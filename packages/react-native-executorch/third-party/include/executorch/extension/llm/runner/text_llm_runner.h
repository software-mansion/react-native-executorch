...
public:
  ...
  explicit TextLLMRunner(
      ...
      std::unique_ptr<Stats> stats, float temperature = -1.0f);
  ...
  prefill(const std::vector<MultimodalInput> &inputs, int32_t num_bos = 0,
   ...
   */
  ::executorch::runtime::Result<uint64_t>
  prefill(const std::string &prompt, const GenerationConfig &config);

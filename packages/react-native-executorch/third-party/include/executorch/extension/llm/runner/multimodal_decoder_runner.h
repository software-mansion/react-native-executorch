...
public:
  ...
  step(executorch::extension::TensorPtr &tokens, int64_t start_pos) override {
    // run token embedding
   ...
   */
  inline executorch::runtime::Result<executorch::aten::Tensor>
  decode(const runtime::EValue &embeddings, int64_t start_pos) {
    ...
    ET_CHECK_MSG(outputs_res.size() == 1,
                 "More then one output returned from executing LLM.");
    ET_CHECK_MSG(outputs_res[0].isTensor(),
  ...
  inline bool is_method_loaded() override {
    ...
    if (!methods_exist) {
      ...
      ET_CHECK_MSG(methods_exist,
                   "Missing required methods (%s, %s) in the model",
                   kTokenEmbeddingMethod, kTextModelMethod);
    }

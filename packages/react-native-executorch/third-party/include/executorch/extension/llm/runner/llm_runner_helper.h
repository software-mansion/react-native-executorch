...
ET_EXPERIMENTAL std::unique_ptr<tokenizers::Tokenizer> load_tokenizer(
    ...
    size_t bos_token_index = 0, size_t eos_token_index = 1);
 ...
 */
ET_EXPERIMENTAL std::unordered_set<uint64_t>
get_eos_ids(tokenizers::Tokenizer *tokenizer, Module *module);
...
ET_EXPERIMENTAL std::unique_ptr<TextLLMRunner> create_text_llm_runner(
    ...
    std::optional<const std::string> data_path, float temperature = -1.0f,
...
ET_EXPERIMENTAL std::unique_ptr<TextLLMRunner> create_text_llm_runner(
    const std::string &model_path,
    std::unique_ptr<::tokenizers::Tokenizer> tokenizer,
    std::vector<std::string> data_files = {}, float temperature = -1.0f,

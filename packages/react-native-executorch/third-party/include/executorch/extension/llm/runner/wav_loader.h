...
inline std::unique_ptr<WavHeader> load_wav_header(const std::string &fp) {
  ...
  if (is_wav_file) {
    ...
    if (data_size >= default_header_size) {
      std::memcpy(reinterpret_cast<char *>(header.get()), data,
                  default_header_size);

      ET_LOG(Info, "WAV header detected, getting raw audio data.");
      ET_LOG(Info, "RIFF Header: %c%c%c%c", header->RIFF[0], header->RIFF[1],
             header->RIFF[2], header->RIFF[3]);
      ET_LOG(Info, "Chunk Size: %d", header->ChunkSize);
      ET_LOG(Info, "WAVE Header: %c%c%c%c", header->WAVE[0], header->WAVE[1],
             header->WAVE[2], header->WAVE[3]);
      ET_LOG(Info, "Format Header: %c%c%c%c", header->fmt[0], header->fmt[1],
             header->fmt[2], header->fmt[3]);
      ...
      if (data_offset != 0) {
        ...
        if (data_offset + 8 > data_size) {
          ET_LOG(Error,
                 "WAV file structure is invalid: data chunk header extends "
                 "beyond file bounds (offset %zu, file size %zu)",
      ...
      ET_CHECK_MSG(false, "WAV header detected but file is too small to "
...
inline std::vector<float> load_wav_audio_data(const std::string &fp) {
  ...
  if (data_size > buffer.size() - data_offset) {
    ET_CHECK_MSG(false,
                 "Invalid WAV file: claimed data size (%zu) exceeds available "
                 "data (%zu bytes from offset %zu)",
                 data_size, buffer.size() - data_offset, data_offset);
  }

  if (audio_format != kWavFormatPcm && audio_format != kWavFormatIeeeFloat) {
    ET_CHECK_MSG(false,
                 "Unsupported audio format: 0x%04X. Only PCM (0x%04X) and IEEE "
                 "Float (0x%04X) are supported.",
                 audio_format, kWavFormatPcm, kWavFormatIeeeFloat);
  }
  ...
  if (bits_per_sample == 32) {
    ...
    for (size_t i = 0; i < num_samples; ++i) {
      audio_data[i] = static_cast<float>(static_cast<double>(input_buffer[i]) *
                                         kOneOverShortMax);
    }
  } else {
    ET_CHECK_MSG(false,
  ...
  ET_LOG(Info, "Loaded %zu audio samples from WAV file: %s", audio_data.size(),
         fp.c_str());

...
/* Division of 64-bit number u_hi:UINT32_C(0) by 32-bit number d, 32-bit
 * quotient output q */
#if defined(__GNUC__) && defined(__i386__) && FXDIV_USE_INLINE_ASSEMBLY
    uint32_t q;
    __asm__("DIVL %[d]" : "=a"(q), "+d"(u_hi) : [d] "r"(d), "a"(0) : "cc");
#elif (defined(_MSC_VER) && _MSC_VER >= 1920) && !defined(__clang__) &&        \
    !defined(__INTEL_COMPILER) && (defined(_M_IX86) || defined(_M_X64))
    unsigned int remainder;
    const uint32_t q = (uint32_t)_udiv64(
        (unsigned __int64)((uint64_t)u_hi << 32), (unsigned int)d, &remainder);
#else
...
static inline struct fxdiv_divisor_uint64_t fxdiv_init_uint64_t(uint64_t d) {
  ...
  if (d == 1) {
    ...
    const uint32_t nlz_d =
        ((uint8_t)l_minus_1 ^ UINT8_C(0x3F)) - d_is_power_of_2;
...
/* Division of 128-bit number u_hi:UINT64_C(0) by 64-bit number d, 64-bit
 * quotient output q */
    ...
    /* GCC, Clang, and Intel Compiler fail to inline optimized implementation
     * and call into support library for 128-bit division */
    const uint64_t q =
        (uint64_t)(((unsigned __int128)u_hi << 64) / ((unsigned __int128)d));
#elif (defined(_MSC_VER) && _MSC_VER >= 1920) && !defined(__clang__) &&        \
    !defined(__INTEL_COMPILER) && defined(_M_X64)
    unsigned __int64 remainder;
    const uint64_t q = (uint64_t)_udiv128((unsigned __int64)u_hi, 0,
                                          (unsigned __int64)d, &remainder);
#else
    ...
    while ((q1 >> 32) != 0 ||
           fxdiv_mulext_uint32_t((uint32_t)q1, d_lo) > (r1 << 32)) {
      q1 -= 1;
      r1 += d_hi;
      if ((r1 >> 32) != 0) {
        break;
    ...
    while ((q0 >> 32) != 0 ||
           fxdiv_mulext_uint32_t((uint32_t)q0, d_lo) > (r0 << 32)) {
      q0 -= 1;
      r0 += d_hi;
      if ((r0 >> 32) != 0) {
        break;
...
#if SIZE_MAX == UINT32_MAX
  const struct fxdiv_divisor_uint32_t uint_result =
      fxdiv_init_uint32_t((uint32_t)d);
#elif SIZE_MAX == UINT64_MAX
  const struct fxdiv_divisor_uint64_t uint_result =
      fxdiv_init_uint64_t((uint64_t)d);
#else
#error Unsupported platform
#endif
  struct fxdiv_divisor_size_t size_result = {(size_t)uint_result.value,
                                             (size_t)uint_result.m,
                                             uint_result.s1, uint_result.s2};
  return size_result;
}

static inline uint32_t
fxdiv_quotient_uint32_t(uint32_t n,
                        const struct fxdiv_divisor_uint32_t divisor) {
  const uint32_t t = fxdiv_mulhi_uint32_t(n, divisor.m);
  return (t + ((n - t) >> divisor.s1)) >> divisor.s2;
}

static inline uint64_t
fxdiv_quotient_uint64_t(uint64_t n,
                        const struct fxdiv_divisor_uint64_t divisor) {
  const uint64_t t = fxdiv_mulhi_uint64_t(n, divisor.m);
  return (t + ((n - t) >> divisor.s1)) >> divisor.s2;
}

static inline size_t
fxdiv_quotient_size_t(size_t n, const struct fxdiv_divisor_size_t divisor) {
#if SIZE_MAX == UINT32_MAX
  const struct fxdiv_divisor_uint32_t uint32_divisor = {
      (uint32_t)divisor.value, (uint32_t)divisor.m, divisor.s1, divisor.s2};
  return fxdiv_quotient_uint32_t((uint32_t)n, uint32_divisor);
#elif SIZE_MAX == UINT64_MAX
  const struct fxdiv_divisor_uint64_t uint64_divisor = {
      (uint64_t)divisor.value, (uint64_t)divisor.m, divisor.s1, divisor.s2};
...
static inline uint32_t
fxdiv_remainder_uint32_t(uint32_t n,
                         const struct fxdiv_divisor_uint32_t divisor) {
...
static inline uint64_t
fxdiv_remainder_uint64_t(uint64_t n,
                         const struct fxdiv_divisor_uint64_t divisor) {
...
static inline size_t
fxdiv_remainder_size_t(size_t n, const struct fxdiv_divisor_size_t divisor) {
  const size_t quotient = fxdiv_quotient_size_t(n, divisor);
  return n - quotient * divisor.value;
}

static inline uint32_t
fxdiv_round_down_uint32_t(uint32_t n,
                          const struct fxdiv_divisor_uint32_t granularity) {
  const uint32_t quotient = fxdiv_quotient_uint32_t(n, granularity);
  return quotient * granularity.value;
}

static inline uint64_t
fxdiv_round_down_uint64_t(uint64_t n,
                          const struct fxdiv_divisor_uint64_t granularity) {
  const uint64_t quotient = fxdiv_quotient_uint64_t(n, granularity);
  return quotient * granularity.value;
}

static inline size_t
fxdiv_round_down_size_t(size_t n,
                        const struct fxdiv_divisor_size_t granularity) {
  const size_t quotient = fxdiv_quotient_size_t(n, granularity);
  return quotient * granularity.value;
}

static inline struct fxdiv_result_uint32_t
fxdiv_divide_uint32_t(uint32_t n, const struct fxdiv_divisor_uint32_t divisor) {
  const uint32_t quotient = fxdiv_quotient_uint32_t(n, divisor);
...
static inline struct fxdiv_result_uint64_t
fxdiv_divide_uint64_t(uint64_t n, const struct fxdiv_divisor_uint64_t divisor) {
  const uint64_t quotient = fxdiv_quotient_uint64_t(n, divisor);
...
static inline struct fxdiv_result_size_t
fxdiv_divide_size_t(size_t n, const struct fxdiv_divisor_size_t divisor) {
  const size_t quotient = fxdiv_quotient_size_t(n, divisor);

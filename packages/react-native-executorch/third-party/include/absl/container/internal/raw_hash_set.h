...
template <typename AllocType>
void CopyAlloc(AllocType &, AllocType &,
...
ShouldInsertBackwards(ABSL_ATTRIBUTE_UNUSED size_t capacity,
...
GetInsertionOffset(Mask mask, ABSL_ATTRIBUTE_UNUSED size_t capacity,
                   ABSL_ATTRIBUTE_UNUSED size_t hash,
                   ABSL_ATTRIBUTE_UNUSED const ctrl_t *ctrl) {
#if defined(NDEBUG)
...
public:
  ...
  operator=(CommonFieldsGenerationInfoEnabled &&) = default;
...
public:
  ...
  operator=(CommonFieldsGenerationInfoDisabled &&) = default;
...
public:
  explicit RawHashSetLayout(size_t capacity, size_t slot_align, bool has_infoz)
      : capacity_(capacity), control_offset_(ControlOffset(has_infoz)),
...
public:
  ...
  HashtablezInfoHandle infoz() {
    return has_infoz() ? *reinterpret_cast<HashtablezInfoHandle *>(
                             backing_array_start())
...
IterateOverFullSlots(const CommonFields &c, SlotType *slot, Callback cb) {
...
HashtablezInfoHandle
SampleHashtablezInfo(size_t sizeof_slot, size_t sizeof_key, size_t sizeof_value,
                     size_t old_capacity, bool was_soo,
                     HashtablezInfoHandle forced_infoz, CommonFields &c) {
...
public:
  explicit HashSetResizeHelper(CommonFields &c, bool was_soo, bool had_soo_slot,
      ...
      : old_capacity_(c.capacity()), had_infoz_(c.has_infoz()),
        was_soo_(was_soo), had_soo_slot_(had_soo_slot),
  ...
  InitializeSlots(CommonFields &c, Alloc alloc, ctrl_t soo_slot_h2,
    ...
    char *mem =
        static_cast<char *>(Allocate<BackingArrayAlignment(AlignOfSlot)>(
...
public:
  ...
  private:
    iterator(ctrl_t *ctrl, slot_type *slot,
             const GenerationType *generation_ptr)
        : HashSetIteratorGenerationInfo(generation_ptr), ctrl_(ctrl),
          slot_(slot) {
  ...
  insert(const T &value) ABSL_ATTRIBUTE_LIFETIME_BOUND {
  ...
  //   s.insert({"abc", 42});
  std::pair<iterator, bool>
  insert(init_type &&value) ABSL_ATTRIBUTE_LIFETIME_BOUND {
  ...
  std::pair<iterator, bool>
  emplace(Args &&...args) ABSL_ATTRIBUTE_LIFETIME_BOUND {
  ...
  equal_range(const key_arg<K> &key) ABSL_ATTRIBUTE_LIFETIME_BOUND {
    auto it = find(key);
    if (it != end())
...
private:
  ...
  resize_impl(CommonFields &common, size_t new_capacity,
              HashtablezInfoHandle forced_infoz) {
  ...
  void
  annotate_for_bug_detection_on_move(ABSL_ATTRIBUTE_UNUSED raw_hash_set &that) {
...
typename raw_hash_set<P, H, E, A>::size_type
EraseIf(Predicate &pred, raw_hash_set<P, H, E, A> *c) {

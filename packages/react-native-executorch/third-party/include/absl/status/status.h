...
private:
  ...
  static absl::Nonnull<status_internal::StatusRep *>
  PrepareToModify(uintptr_t rep);
  ...
  static absl::Nonnull<const status_internal::StatusRep *>
  RepToPointer(uintptr_t r);
...
inline absl::optional<absl::Cord>
Status::GetPayload(absl::string_view type_url) const {
  if (IsInlined(rep_))
    return absl::nullopt;
...
inline absl::Nonnull<const status_internal::StatusRep *>
Status::RepToPointer(uintptr_t rep) {
  assert(!IsInlined(rep));
  return reinterpret_cast<const status_internal::StatusRep *>(rep);
}

inline uintptr_t
Status::PointerToRep(absl::Nonnull<status_internal::StatusRep *> rep) {
...
StatusMessageAsCStr(const Status &status ABSL_ATTRIBUTE_LIFETIME_BOUND);

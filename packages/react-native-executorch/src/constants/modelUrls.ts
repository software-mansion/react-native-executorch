...
/**
 * Registry of all available model configurations, organized by category.
 *
 * Each model appears in its semantic subcategory and in `ALL_MODELS`.
 * @example
 * ```ts
 * import { MODEL_REGISTRY } from 'react-native-executorch';
 *
 * // Access a specific model
 * const llama = MODEL_REGISTRY.LLM.LLAMA3_2_1B;
 *
 * // List all model names
 * const names = Object.values(MODEL_REGISTRY.ALL_MODELS).map(m => m.modelName);
 *
 * // List all speech-to-text models
 * const sttModels = Object.values(MODEL_REGISTRY.SPEECH_TO_TEXT);
 * ```
 * @category Utils
 */

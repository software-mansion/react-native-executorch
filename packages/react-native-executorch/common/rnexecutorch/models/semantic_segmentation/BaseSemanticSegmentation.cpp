...
BaseSemanticSegmentation::BaseSemanticSegmentation(const std::string &modelSource,
                                                   std::vector<float> normMean,
                                                   std::vector<float> normStd,
                                                   std::vector<std::string> allClasses,
                                                   std::shared_ptr<react::CallInvoker> callInvoker)
    : VisionModel(modelSource, callInvoker), allClasses_(std::move(allClasses)) {
  initModelImageSize();
  if (normMean.size() == 3) {
...
BaseSemanticSegmentation::runInference(cv::Mat image, cv::Size originalSize,
                                       std::set<std::string, std::less<>> &classesOfInterest,
                                       bool resize) {
  std::scoped_lock lock(inference_mutex_);

  cv::Mat preprocessed = VisionModel::preprocess(image);
  auto inputTensor = (normMean_ && normStd_)
  ...
  return computeResult(forwardResult->at(0).toTensor(), originalSize, allClasses_,
                       classesOfInterest, resize);
}

semantic_segmentation::SegmentationResult BaseSemanticSegmentation::generateFromString(
    std::string imageSource, std::set<std::string, std::less<>> classesOfInterest, bool resize) {
...
semantic_segmentation::SegmentationResult BaseSemanticSegmentation::generateFromPixels(
    JSTensorViewIn pixelData, std::set<std::string, std::less<>> classesOfInterest, bool resize) {
  cv::Mat image = extractFromPixels(pixelData);
  return runInference(image, image.size(), classesOfInterest, resize);
}

semantic_segmentation::SegmentationResult
BaseSemanticSegmentation::generateFromFrame(jsi::Runtime &runtime, const jsi::Value &frameData,
                                            std::set<std::string, std::less<>> classesOfInterest,
                                            bool resize) {
  ...
  auto inverseAndResize = [&orient, &frameSize, &outputSize,
                           resize](std::shared_ptr<OwningArrayBuffer> &buf, int32_t cvType,
                                   int32_t interpFlag) {
    ...
    buf = std::make_shared<OwningArrayBuffer>(inv.data,
                                              static_cast<size_t>(inv.total() * inv.elemSize()));
  };
...
semantic_segmentation::SegmentationResult BaseSemanticSegmentation::computeResult(
    const Tensor &tensor, cv::Size originalSize, std::vector<std::string> &allClasses,
  ...
  std::size_t numChannels = (tensor.dim() >= 3) ? tensor.size(tensor.dim() - 3) : 1;
  ...
  if (numChannels == 1) {
    ...
    auto fg = std::make_shared<OwningArrayBuffer>(resultData.data(), outputPixels * sizeof(float));
  ...
  auto argmax = std::make_shared<OwningArrayBuffer>(outputPixels * sizeof(int32_t));
  ...
  if (numChannels == 1) {
    ...
    std::vector<float> maxLogits(outputPixels, -std::numeric_limits<float>::infinity());
  ...
  auto buffersToReturn =
      std::make_shared<std::unordered_map<std::string, std::shared_ptr<OwningArrayBuffer>>>();
  ...
  if (resize) {
    ...
    cv::resize(argmaxMat, argmaxMat, originalSize, 0, 0, cv::InterpolationFlags::INTER_NEAREST);
    argmax =
        std::make_shared<OwningArrayBuffer>(argmaxMat.data, originalSize.area() * sizeof(int32_t));
    ...
    for (auto &[label, arrayBuffer] : *buffersToReturn) {
      ...
      arrayBuffer =
          std::make_shared<OwningArrayBuffer>(classMat.data, originalSize.area() * sizeof(float));
    }
  }

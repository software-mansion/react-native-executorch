...
ObjectDetection::ObjectDetection(const std::string &modelSource, std::vector<float> normMean,
...
void ObjectDetection::ensureMethodLoaded(const std::string &methodName) {
  if (methodName.empty()) {
    throw RnExecutorchError(RnExecutorchErrorCode::InvalidUserInput, "methodName cannot be empty");
  }
  if (currentlyLoadedMethod_ == methodName) {
    return;
  }
  if (!module_) {
    throw RnExecutorchError(RnExecutorchErrorCode::ModuleNotLoaded, "Model module is not loaded");
  }
  if (!currentlyLoadedMethod_.empty()) {
...
std::set<int32_t>
ObjectDetection::prepareAllowedClasses(const std::vector<int32_t> &classIndices) const {
...
ObjectDetection::postprocess(const std::vector<EValue> &tensors, cv::Size originalSize,
  ...
  float heightRatio = static_cast<float>(originalSize.height) / inputSize.height;
  ...
  std::span<const float> bboxes(static_cast<const float *>(bboxTensor.const_data_ptr()),
                                bboxTensor.numel());

  auto scoreTensor = tensors.at(1).toTensor();
  std::span<const float> scores(static_cast<const float *>(scoreTensor.const_data_ptr()),
                                scoreTensor.numel());

  auto labelTensor = tensors.at(2).toTensor();
  std::span<const float> labels(static_cast<const float *>(labelTensor.const_data_ptr()),
                                labelTensor.numel());
  ...
  for (std::size_t i = 0; i < scores.size(); ++i) {
    ...
    if (!allowedClasses.empty() && allowedClasses.find(labelIdx) == allowedClasses.end()) {
      continue;
    }
    ...
    detections.emplace_back(utils::computer_vision::BBox{x1, y1, x2, y2}, labelNames_[labelIdx],
                            labelIdx, scores[i]);
  }
...
std::vector<types::Detection>
ObjectDetection::runInference(cv::Mat image, double detectionThreshold, double iouThreshold,
                              const std::vector<int32_t> &classIndices,
  ...
  return postprocess(executeResult.get(), originalSize, detectionThreshold, iouThreshold,
                     classIndices);
}

std::vector<types::Detection> ObjectDetection::generateFromString(std::string imageSource,
  ...
  return runInference(imageRGB, detectionThreshold, iouThreshold, classIndices, methodName);
}

std::vector<types::Detection>
ObjectDetection::generateFromFrame(jsi::Runtime &runtime, const jsi::Value &frameData,
                                   double detectionThreshold, double iouThreshold,
  ...
  auto detections =
      runInference(rotated, detectionThreshold, iouThreshold, classIndices, methodName);
...
std::vector<types::Detection> ObjectDetection::generateFromPixels(JSTensorViewIn pixelData,
                                                                  double detectionThreshold,
                                                                  double iouThreshold,
                                                                  std::vector<int32_t> classIndices,
                                                                  std::string methodName) {
  ...
  return runInference(image, detectionThreshold, iouThreshold, classIndices, methodName);
}
} // namespace rnexecutorch::models::object_detection

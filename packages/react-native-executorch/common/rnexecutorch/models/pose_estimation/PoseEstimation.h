...
public:
  ...
  generateFromString(std::string imageSource, double detectionThreshold, double keypointThreshold,
  ...
  [[nodiscard("Registered non-void function")]] PoseDetections
  generateFromPixels(JSTensorViewIn pixelData, double detectionThreshold, double keypointThreshold,
                     std::string methodName);
...
private:
  ...
  PoseDetections runInference(cv::Mat image, double detectionThreshold, double keypointThreshold,
                              const std::string &modelName);

  [[nodiscard("Registered non-void function")]]
  PoseDetections postprocess(const std::vector<EValue> &evl, cv::Size originalSize,
                             double detectionThreshold, double keypointThreshold);
};

} // namespace models::pose_estimation

REGISTER_CONSTRUCTOR(models::pose_estimation::PoseEstimation, std::string, std::vector<float>,
                     std::vector<float>, std::shared_ptr<react::CallInvoker>);
} // namespace rnexecutorch

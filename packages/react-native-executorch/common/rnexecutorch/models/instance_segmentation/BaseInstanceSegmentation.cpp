...
BaseInstanceSegmentation::BaseInstanceSegmentation(const std::string &modelSource,
...
std::vector<types::Instance> BaseInstanceSegmentation::runInference(
    const cv::Mat &image, double confidenceThreshold, double iouThreshold, int32_t maxInstances,
    const std::vector<int32_t> &classIndices, bool returnMaskAtOriginalResolution,
  ...
  if (inputShapes.empty() || inputShapes[0].empty()) {
    throw RnExecutorchError(RnExecutorchErrorCode::UnexpectedNumInputs,
                            "Method '" + methodName + "' has invalid input tensor shape.");
  }
  ...
  auto forwardResult = BaseModel::execute(methodName, {buildInputTensor(image)});
  CHECK_OK_OR_THROW_FORWARD_ERROR(forwardResult);

  validateOutputTensors(forwardResult.get());

  auto instances =
      collectInstances(forwardResult.get(), originalSize, modelInputSize, confidenceThreshold,
...
std::vector<types::Instance> BaseInstanceSegmentation::generateFromString(
    std::string imageSource, double confidenceThreshold, double iouThreshold, int32_t maxInstances,
  ...
  return runInference(imageRGB, confidenceThreshold, iouThreshold, maxInstances, classIndices,
                      returnMaskAtOriginalResolution, methodName);
}

std::vector<types::Instance> BaseInstanceSegmentation::generateFromFrame(
    jsi::Runtime &runtime, const jsi::Value &frameData, double confidenceThreshold,
    double iouThreshold, int32_t maxInstances, std::vector<int32_t> classIndices,
  ...
  auto instances = runInference(rotated, confidenceThreshold, iouThreshold, maxInstances,
  ...
  for (auto &inst : instances) {
    ...
    inst.mask =
        std::make_shared<OwningArrayBuffer>(invMask.data, static_cast<size_t>(invMask.total()));
...
std::vector<types::Instance> BaseInstanceSegmentation::generateFromPixels(
    ...
    int32_t maxInstances, std::vector<int32_t> classIndices, bool returnMaskAtOriginalResolution,
    std::string methodName) {

  cv::Mat image = extractFromPixels(tensorView);
  return runInference(image, confidenceThreshold, iouThreshold, maxInstances, classIndices,
                      returnMaskAtOriginalResolution, methodName);
}

std::tuple<utils::computer_vision::BBox, float, int32_t>
BaseInstanceSegmentation::extractDetectionData(const float *bboxData, const float *scoresData,
                                               int32_t index) {
  utils::computer_vision::BBox bbox{bboxData[index * 4], bboxData[index * 4 + 1],
...
cv::Rect
BaseInstanceSegmentation::computeMaskCropRect(const utils::computer_vision::BBox &bboxModel,
  ...
  int32_t my2 = std::min(maskSize.height, static_cast<int32_t>(std::ceil(my2F)));

  return {mx1, my1, mx2 - mx1, my2 - my1};
}

cv::Rect BaseInstanceSegmentation::addPaddingToRect(const cv::Rect &rect, cv::Size maskSize) {
...
cv::Mat BaseInstanceSegmentation::warpToOriginalResolution(
    const cv::Mat &probMat, const cv::Rect &maskRect, cv::Size originalSize, cv::Size maskSize,
  ...
  cv::Mat M = (cv::Mat_<float>(2, 3) << scaleX, 0, (maskRect.x * scaleX - bboxOriginal.p1.x), 0,
               scaleY, (maskRect.y * scaleY - bboxOriginal.p1.y));
...
cv::Mat BaseInstanceSegmentation::processMaskFromLogits(
  ...
  if (warpToOriginal) {
    probMat = warpToOriginalResolution(probMat, cropRect, originalSize, maskSize, bboxOriginal);
  }
...
void BaseInstanceSegmentation::validateOutputTensors(const std::vector<EValue> &tensors) const {
  if (tensors.size() != 3) {
...
std::set<int32_t>
BaseInstanceSegmentation::prepareAllowedClasses(const std::vector<int32_t> &classIndices) const {
...
void BaseInstanceSegmentation::ensureMethodLoaded(const std::string &methodName) {
  if (methodName.empty()) {
    throw RnExecutorchError(RnExecutorchErrorCode::InvalidConfig,
  ...
  if (!module_) {
    throw RnExecutorchError(RnExecutorchErrorCode::ModuleNotLoaded,
                            "Model not loaded. Cannot load method '" + methodName + "'.");
  }
  ...
  if (loadResult != executorch::runtime::Error::Ok) {
    throw RnExecutorchError(loadResult, "Failed to load method '" + methodName +
...
BaseInstanceSegmentation::finalizeInstances(std::vector<types::Instance> instances,
                                            double iouThreshold, int32_t maxInstances) const {

  if (applyNMS_) {
    instances = utils::computer_vision::nonMaxSuppression(instances, iouThreshold);
  }
...
std::vector<types::Instance> BaseInstanceSegmentation::collectInstances(
    const std::vector<EValue> &tensors, cv::Size originalSize, cv::Size modelInputSize,
    double confidenceThreshold, const std::vector<int32_t> &classIndices,
    bool returnMaskAtOriginalResolution) {

  float widthRatio = static_cast<float>(originalSize.width) / modelInputSize.width;
  float heightRatio = static_cast<float>(originalSize.height) / modelInputSize.height;
  ...
  auto isValidDetection = [&allowedClasses, &confidenceThreshold](float score, int32_t labelIdx) {
  ...
  for (int32_t i = 0; i < numInstances; ++i) {
    auto [bboxModel, score, labelIdx] = extractDetectionData(bboxData, scoresData, i);

    if (!isValidDetection(score, labelIdx)) {
      continue;
    }

    utils::computer_vision::BBox bboxOriginal = bboxModel.scale(widthRatio, heightRatio);
    if (!bboxOriginal.isValid()) {
      continue;
    }

    cv::Mat logitsMat(maskH, maskW, CV_32FC1, const_cast<float *>(maskData + (i * maskH * maskW)));

    cv::Mat binaryMask = processMaskFromLogits(logitsMat, bboxModel, bboxOriginal, modelInputSize,
                                               originalSize, returnMaskAtOriginalResolution);

    instances.emplace_back(bboxOriginal,
                           std::make_shared<OwningArrayBuffer>(binaryMask.data, binaryMask.total()),

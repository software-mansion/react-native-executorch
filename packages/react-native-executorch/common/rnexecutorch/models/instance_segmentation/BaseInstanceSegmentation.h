...
public:
  BaseInstanceSegmentation(const std::string &modelSource, std::vector<float> normMean,
                           std::vector<float> normStd, bool applyNMS,
                           std::shared_ptr<react::CallInvoker> callInvoker);

  [[nodiscard("Registered non-void function")]] std::vector<types::Instance>
  generateFromString(std::string imageSource, double confidenceThreshold, double iouThreshold,
                     int32_t maxInstances, std::vector<int32_t> classIndices,
                     bool returnMaskAtOriginalResolution, std::string methodName);

  [[nodiscard("Registered non-void function")]] std::vector<types::Instance>
  generateFromFrame(jsi::Runtime &runtime, const jsi::Value &frameData, double confidenceThreshold,
                    double iouThreshold, int32_t maxInstances, std::vector<int32_t> classIndices,
                    bool returnMaskAtOriginalResolution, std::string methodName);

  [[nodiscard("Registered non-void function")]] std::vector<types::Instance>
  generateFromPixels(JSTensorViewIn tensorView, double confidenceThreshold, double iouThreshold,
                     int32_t maxInstances, std::vector<int32_t> classIndices,
                     bool returnMaskAtOriginalResolution, std::string methodName);
...
private:
  std::vector<types::Instance> runInference(const cv::Mat &image, double confidenceThreshold,
  ...
  std::vector<types::Instance> collectInstances(const std::vector<EValue> &tensors,
                                                cv::Size originalSize, cv::Size modelInputSize,
                                                double confidenceThreshold,
                                                const std::vector<int32_t> &classIndices,
                                                bool returnMaskAtOriginalResolution);

  void validateThresholds(double confidenceThreshold, double iouThreshold) const;
  ...
  extractDetectionData(const float *bboxData, const float *scoresData, int32_t index);
  ...
  cv::Mat warpToOriginalResolution(const cv::Mat &probMat, const cv::Rect &maskRect,
  ...
  std::vector<types::Instance> finalizeInstances(std::vector<types::Instance> instances,
                                                 double iouThreshold, int32_t maxInstances) const;

  cv::Mat processMaskFromLogits(const cv::Mat &logitsMat,
                                const utils::computer_vision::BBox &bboxModel,
                                const utils::computer_vision::BBox &bboxOriginal,
...
REGISTER_CONSTRUCTOR(models::instance_segmentation::BaseInstanceSegmentation, std::string,

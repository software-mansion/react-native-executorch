...
public:
  ...
  std::pair<std::vector<int32_t>, float> generate(const cv::Mat &grayImage, int32_t inputWidth);

private:
  std::pair<std::vector<int32_t>, float> postprocess(const Tensor &tensor) const;
};
} // namespace rnexecutorch::models::ocr

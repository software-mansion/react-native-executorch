...
Detector::Detector(const std::string &modelSource, std::shared_ptr<react::CallInvoker> callInvoker)
  ...
  for (auto input_size : constants::kDetectorInputWidths) {
    ...
    if (inputShapes[0].size() < 2) {
      std::string errorMessage = "Unexpected detector model input size for method: " + methodName +
...
std::vector<types::DetectorBBox> Detector::generate(const cv::Mat &inputImage, int32_t inputWidth) {
  /*
  ...
  utils::validateInputWidth(inputWidth, constants::kDetectorInputWidths, "Detector");
...
cv::Size Detector::calculateModelImageSize(int32_t methodInputWidth) {
  ...
  utils::validateInputWidth(methodInputWidth, constants::kDetectorInputWidths, "Detector");
  std::string methodName = "forward_" + std::to_string(methodInputWidth);

  auto inputShapes = getAllInputShapes(methodName);
  std::vector<int32_t> modelInputShape = inputShapes[0];
  cv::Size modelInputSize = cv::Size(modelInputShape[modelInputShape.size() - 1],
                                     modelInputShape[modelInputShape.size() - 2]);
  return modelInputSize;
}

std::vector<types::DetectorBBox> Detector::postprocess(const Tensor &tensor,
  ...
  */
  std::vector<types::DetectorBBox> bBoxesList =
      utils::getDetBoxesFromTextMap(scoreTextMat, scoreAffinityMat, constants::kTextThreshold,
  ...
  */
  bBoxesList =
      utils::groupTextBoxes(bBoxesList, constants::kCenterThreshold, constants::kDistanceThreshold,

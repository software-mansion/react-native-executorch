...
std::pair<std::vector<int32_t>, float> Recognizer::generate(const cv::Mat &grayImage,
                                                            int32_t inputWidth) {
  /*
  ...
  */
  utils::validateInputWidth(inputWidth, constants::kRecognizerInputWidths, "Recognizer");
  ...
  TensorPtr inputTensor = image_processing::getTensorFromMatrixGray(tensorDims, grayImage);
...
std::pair<std::vector<int32_t>, float> Recognizer::postprocess(const Tensor &tensor) const {
  /*
  ...
  */
  ...
  cv::Mat resultMat(numRows, alphabetSize, CV_32F, tensor.mutable_data_ptr<float>());

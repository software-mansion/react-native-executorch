...
Decoder::Decoder(const std::string &modelSource, std::shared_ptr<react::CallInvoker> callInvoker)
    : BaseModel(modelSource, callInvoker) {}

std::vector<float> Decoder::generate(std::vector<float> &input) const {
  std::vector<int32_t> inputShape = {1, numChannels, latentImageSize, latentImageSize};
  auto inputTensor = make_tensor_ptr(inputShape, input.data(), ScalarType::Float);

...
TextToImage::TextToImage(const std::string &tokenizerSource, const std::string &encoderSource,
    ...
    : callInvoker(callInvoker),
      scheduler(std::make_unique<Scheduler>(schedulerBetaStart, schedulerBetaEnd,
                                            schedulerNumTrainTimesteps, schedulerStepsOffset,
                                            callInvoker)),
      encoder(std::make_unique<Encoder>(tokenizerSource, encoderSource, callInvoker)),
...
std::string TextToImage::generate(std::string input, int32_t imageSize, size_t numInferenceSteps,
  ...
  auto embeddingsTensor = make_tensor_ptr(embeddingsShape, embeddings.data(), ScalarType::Float);
  ...
  for (size_t t = 0; t < numInferenceSteps + 1 && !interrupted; t++) {
    ...
    std::vector<float> noisePred = unet->generate(latents, timesteps[t], embeddingsTensor);
    ...
    std::span<const float> noiseText = noisePredSpan.subspan(noiseSize, noiseSize);
...
std::string TextToImage::postprocess(const std::vector<float> &output) const {
  ...
  for (int32_t y = 0; y < imageSize; ++y) {
    ...
    for (int32_t x = 0; x < imageSize; ++x) {
      ...
      row[x] =
          cv::Vec3b(static_cast<uint8_t>(output[idx + 2]), static_cast<uint8_t>(output[idx + 1]),

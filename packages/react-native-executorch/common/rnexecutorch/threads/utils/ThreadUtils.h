...
void unsafeSetupThreadPool(uint32_t num_of_cores = 0) {
  auto num_of_perf_cores = ::executorch::extension::cpuinfo::get_num_performant_cores();
  ...
  // performance cores
  auto _num_of_cores =
      num_of_cores ? num_of_cores : static_cast<uint32_t>(num_of_perf_cores / 2) + 1;
  const auto threadpool = ::executorch::extension::threadpool::get_threadpool();
  threadpool->_unsafe_reset_threadpool(_num_of_cores);
  log(LOG_LEVEL::Info, "Configuring xnnpack for", threadpool->get_thread_count(), "threads");
}

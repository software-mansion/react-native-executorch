...
#define JSI_HOST_FUNCTION(NAME)                                                                    \
  jsi::Value NAME(jsi::Runtime &runtime, const jsi::Value &thisValue, const jsi::Value *args,      \
                  size_t count)

#define JSI_EXPORT_FUNCTION(CLASS, FUNCTION, NAME)                                                 \
  std::make_pair(NAME, static_cast<jsi::Value (JsiHostObject::*)(                                  \
...
#define JSI_PROPERTY_SETTER(name) void name(jsi::Runtime &runtime, const jsi::Value &value)
...
public:
  ...
  template <typename... Args> void addGetters(Args... args) { (getters_->insert(args), ...); }

  template <typename... Args> void addSetters(Args... args) { (setters_->insert(args), ...); }

  template <typename... Args> void addFunctions(Args... args) { (functions_->insert(args), ...); }

...
std::string formatChatML(const std::string &systemPrompt, const std::string &userMessage) {
...
TEST(LLMCtorTests, InvalidTokenizerPathThrows) {
  EXPECT_THROW(LLM(kValidModelPath, "nonexistent_tokenizer.json", {}, createMockCallInvoker()),
               RnExecutorchError);
}

TEST(LLMCtorTests, WrongCapabilitiesThrowsClearError) {
  EXPECT_THROW(LLM(kValidModelPath, kValidTokenizerPath, {"vision"}, createMockCallInvoker()),
...
TEST_F(LLMTest, GenerateProducesValidOutput) {
  ...
  std::string prompt = formatChatML(kSystemPrompt, "Repeat exactly this: `naszponcilem testy`");
...
TEST(VisionEncoderTest, LoadFailsWithClearErrorWhenMethodMissing) {
  ...
  auto module = std::make_unique<::executorch::extension::Module>(
      "smolLm2_135M_8da4w.pte", ::executorch::extension::Module::LoadMode::File);

  auto encoder = std::make_unique<executorch::extension::llm::VisionEncoder>(*module);
...
TEST(AudioEncoderTest, LoadFailsWithClearErrorWhenMethodMissing) {
  ...
  auto encoder = std::make_unique<executorch::extension::llm::AudioEncoder>(*module);
...
constexpr auto kTestImagePath = "file:///data/local/tmp/rnexecutorch_tests/test_image.jpg";
...
TEST_F(LLMTest, GenerateMultimodalOnTextModelThrows) {
  ...
  MultimodalInputs inputs{.images = ImageInputs{.paths = {}, .token = "<image>"}};
  EXPECT_THROW(model.generateMultimodal("hello", nullptr, std::move(inputs)), RnExecutorchError);
}
...
protected:
  static void SetUpTestSuite() {
    ...
    model_ = std::make_unique<LLM>(kVlmModelPath, kVlmTokenizerPath,
...
TEST_F(VLMTest, GenerateMultimodalEmptyImageTokenThrows) {
  MultimodalInputs inputs{.images = ImageInputs{.paths = {kTestImagePath}, .token = ""}};
  EXPECT_THROW(model_->generateMultimodal("hello", nullptr, std::move(inputs)), RnExecutorchError);
}
...
TEST_F(VLMTest, GenerateMultimodalMorePlaceholdersThanImagePaths) {
  ...
  MultimodalInputs inputs{.images =
                              ImageInputs{.paths = {kTestImagePath}, .token = kVlmImageToken}};
  EXPECT_THROW(model_->generateMultimodal(prompt, nullptr, std::move(inputs)), RnExecutorchError);
}
...
TEST_F(VLMTest, GenerateMultimodalMoreImagePathsThanPlaceholders) {
  ...
  MultimodalInputs inputs{
      .images = ImageInputs{.paths = {kTestImagePath, kTestImagePath}, .token = kVlmImageToken}};
  EXPECT_THROW(model_->generateMultimodal(prompt, nullptr, std::move(inputs)), RnExecutorchError);
}
...
protected:
  static void SetUpTestSuite() {
    ...
    model_ = std::make_unique<LLM>(kGemmaModelPath, kGemmaTokenizerPath,
                                   std::vector<std::string>{"vision", "audio"}, invoker_);
  }
...
TEST_F(GemmaAudioTest, GenerateMultimodalNoInputsThrows) {
  EXPECT_THROW(model_->generateMultimodal("hello", nullptr, {}), RnExecutorchError);
}

TEST_F(GemmaAudioTest, GenerateMultimodalEmptyAudioTokenThrows) {
  MultimodalInputs inputs{.audios = AudioInputs{.waveforms = {loadAudio()}, .token = ""}};
  EXPECT_THROW(model_->generateMultimodal("hello", nullptr, std::move(inputs)), RnExecutorchError);
}

TEST_F(GemmaAudioTest, GenerateMultimodalMorePlaceholdersThanWaveformsThrows) {
  std::string prompt = std::string(kGemmaAudioToken) + " and " + kGemmaAudioToken;
  MultimodalInputs inputs{.audios =
                              AudioInputs{.waveforms = {loadAudio()}, .token = kGemmaAudioToken}};
  EXPECT_THROW(model_->generateMultimodal(prompt, nullptr, std::move(inputs)), RnExecutorchError);
}
...
TEST_F(GemmaAudioTest, GenerateMultimodalMoreWaveformsThanPlaceholdersThrows) {
  ...
  MultimodalInputs inputs{
      .audios = AudioInputs{.waveforms = {loadAudio(), loadAudio()}, .token = kGemmaAudioToken}};
  EXPECT_THROW(model_->generateMultimodal(prompt, nullptr, std::move(inputs)), RnExecutorchError);
}

TEST_F(GemmaAudioTest, GenerateMultimodalAudioProducesOutput) {
  std::vector<float> wav = loadAudio();
  ASSERT_FALSE(wav.empty()) << "test_audio_float.raw missing on device - check run_tests.sh assets";

  std::string prompt = formatGemma(std::string(kGemmaAudioToken) + " Transcribe the audio.");
  MultimodalInputs inputs{
      .audios = AudioInputs{.waveforms = {std::move(wav)}, .token = kGemmaAudioToken}};
  std::string output = model_->generateMultimodal(prompt, nullptr, std::move(inputs));
...
TEST_F(GemmaAudioTest, GenerateMultimodalInterleavedTextAndAudio) {
  std::string prompt =
      formatGemma("Listen: " + std::string(kGemmaAudioToken) + " then summarise it.");
  MultimodalInputs inputs{.audios =
                              AudioInputs{.waveforms = {loadAudio()}, .token = kGemmaAudioToken}};
  std::string output = model_->generateMultimodal(prompt, nullptr, std::move(inputs));

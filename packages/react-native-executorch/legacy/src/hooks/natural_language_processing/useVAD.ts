...
export const useVAD = ({ model, preventLoad = false }: VADProps): VADType => {
  const { error, isReady, isGenerating, downloadProgress, runForward, runSideChannel } =
    useModuleFactory({
      factory: (config, onProgress) => VADModule.fromModelName(config, onProgress),
      config: model,
      deps: [model.modelName, model.modelSource],
      preventLoad,
    });

  const forward = (waveform: Float32Array) => runForward((inst) => inst.forward(waveform));

  const stream = (input: VADStreamingInput) => runForward((inst) => inst.stream(input));

...
export const useTextToSpeech = (
  ...
  const [moduleInstance, setModuleInstance] = useState<TextToSpeechModule | null>(null);
  ...
  const guardReady = useCallback(
    (methodName: string): TextToSpeechModule => {
      ...
      if (isGenerating) throw new RnExecutorchError(RnExecutorchErrorCode.ModelGenerating);
  ...
  const forward = async (input: TextToSpeechInput) => {
    ...
    try {
      ...
      return await instance.forward(input.text ?? '', input.speed ?? 1.0, input.phonemize ?? true);
    } finally {
      ...
      try {
        if (input.text) {
          instance.streamInsert(
            input.text + ('.?!;'.includes(input.text.trim().slice(-1)) ? '' : '.')
          );
        }

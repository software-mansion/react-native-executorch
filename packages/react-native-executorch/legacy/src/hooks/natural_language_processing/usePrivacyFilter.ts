...
import { PiiEntity, PrivacyFilterProps, PrivacyFilterType } from '../../types/privacyFilter';
...
export const usePrivacyFilter = ({
  ...
  const { error, isReady, isGenerating, downloadProgress, runForward } = useModuleFactory({
    factory: (config, onProgress) => PrivacyFilterModule.fromModelName(config, onProgress),
    config: model,

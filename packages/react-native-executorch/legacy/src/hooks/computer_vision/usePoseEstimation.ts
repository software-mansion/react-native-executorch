...
export const usePoseEstimation = <C extends PoseEstimationModelSources>({
  model,
  preventLoad = false,
}: PoseEstimationProps<C>): PoseEstimationType<PoseEstimationKeypoints<C['modelName']>> => {
  const { error, isReady, isGenerating, downloadProgress, runForward, runOnFrame, instance } =
    useModuleFactory({
      factory: (config, onProgress) => PoseEstimationModule.fromModelName(config, onProgress),
      config: model,
      deps: [model.modelName, model.modelSource],
      preventLoad,
    });

  const forward = (input: string | PixelData, options?: PoseEstimationOptions) =>
    runForward((inst) => inst.forward(input, options));

  const getAvailableInputSizes = () => instance?.getAvailableInputSizes() ?? undefined;

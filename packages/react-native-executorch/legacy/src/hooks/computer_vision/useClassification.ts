...
export const useClassification = <C extends ClassificationModelSources>({
  model,
  preventLoad = false,
}: ClassificationProps<C>): ClassificationType<ClassificationLabels<C['modelName']>> => {
  const { error, isReady, isGenerating, downloadProgress, runForward, runOnFrame } =
    useModuleFactory({
      factory: (config, onProgress) => ClassificationModule.fromModelName(config, onProgress),
      config: model,
      deps: [model.modelName, model.modelSource],
      preventLoad,
    });

  const forward = (input: string | PixelData) => runForward((inst) => inst.forward(input));

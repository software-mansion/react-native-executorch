...
> => {
  const { error, isReady, isGenerating, downloadProgress, runForward, instance, runOnFrame } =
    useModuleFactory({
  ...
  const forward: InstanceSegmentationType<InstanceSegmentationLabels<C['modelName']>>['forward'] = (
    imageSource,
    options

...
export const useOCR = ({ model, preventLoad = false }: OCRProps): OCRType => {
  ...
  useEffect(() => {
    ...
    controller
      .load(model.detectorSource, model.recognizerSource, model.language, setDownloadProgress)
      .then(() => {
  ...
  const forward = useCallback(
    (imageSource: string | PixelData): Promise<OCRDetection[]> => controller.forward(imageSource),
    [controller]
  );

...
export class LLMController {
  ...
  public async load({
    ...
    try {
      ...
      this.nativeModule = await global.loadLLM(modelPath, tokenizerPath, model.capabilities ?? []);
  ...
  private applyGenerationConfig(generationConfig: GenerationConfig) {
    ...
    if (generationConfig.repetitionPenalty !== undefined) {
      ...
      this.nativeModule.setRepetitionPenalty(generationConfig.repetitionPenalty);
    }
  }
  ...
  private filterSpecialTokens(text: string): string {
    ...
    if (SPECIAL_TOKENS.EOT_TOKEN in this.tokenizerConfig && this.tokenizerConfig.eot_token) {
  ...
  public async forward(
    ...
    try {
      ...
      if (hasImages || hasAudio) {
        response = await this.nativeModule.generateMultimodal(input, this.onToken, {
  ...
  public async generate(messages: Message[], tools?: LLMTool[]): Promise<string> {
    if (!this._isReady) {
      throw new RnExecutorchError(
        RnExecutorchErrorCode.ModuleNotLoaded,
    ...
    const imagePaths = messages.filter((m) => m.mediaPath).map((m) => m.mediaPath!);
    const audioWaveforms = messages.filter((m) => m.audioWaveform).map((m) => m.audioWaveform!);
    const renderedChat: string = this.applyChatTemplate(messages, this.tokenizerConfig, tools, {
  ...
  public async sendMessage(
    ...
    const countTokensCallback = (messages: Message[]) => {
      ...
      return textTokens + imageCount * (visualTokenCount - 1) + audioTokenExpansion;
    };
    const maxContextLength = this.nativeModule.getMaxContextLength();
    const messageHistoryWithPrompt = this.chatConfig.contextStrategy.buildContext(
    ...
    if (this.toolsConfig) {
      try {
        ...
        for (const toolCall of toolCalls) {
          const toolResponse = await this.toolsConfig.executeToolCallback(toolCall);

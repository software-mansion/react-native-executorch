...
export class TextToSpeechModule {
  ...
  static async fromModelName(
    ...
    try {
      const nativeModule = await TextToSpeechModule.loadKokoro(config, onDownloadProgress);
  ...
  private static async loadKokoro(
    ...
    const kokoroModel = model as Extract<TextToSpeechModelSources, { modelName: 'kokoro' }>;
    ...
    const paths = await ResourceFetcher.fetch(onDownloadProgressCallback, ...sources);

...
export class SpeechToTextModule {
  ...
  private constructor(nativeModule: unknown, modelConfig: SpeechToTextModelConfig) {
   ...
   */
  public async decode(tokens: Int32Array, encoderOutput: Float32Array): Promise<Float32Array> {
    ...
    (async () => {
      try {
        await this.nativeModule.stream(
          (committed: TranscriptionResult, nonCommitted: TranscriptionResult, isDone: boolean) => {
            queue.push({
              committed,
              nonCommitted,
            });

...
} as const satisfies Record<ObjectDetectionModelName, ObjectDetectionConfig<LabelEnum>>;
 ...
 * @category Types
 */
export type ObjectDetectionLabels<M extends ObjectDetectionModelName> = ResolveLabelsFor<
  M,
  ModelConfigsType
>;
...
/** @internal */
type ResolveLabels<T extends ObjectDetectionModelName | LabelEnum> = ResolveLabelsFor<
  T,
  ModelConfigsType
>;
...
> extends VisionLabeledModule<Detection<ResolveLabels<T>>[], ResolveLabels<T>> {
  ...
  static async fromModelName<C extends ObjectDetectionModelSources>(
    ...
    const modelConfig = ModelConfigs[namedSources.modelName] as ObjectDetectionConfig<LabelEnum>;
  ...
  override get runOnFrame(): (
    ...
    return (
      ...
      if (
        availableInputSizes &&
        inputSize !== undefined &&
        !availableInputSizes.includes(inputSize as (typeof availableInputSizes)[number])
      ) {
        throw new Error(
  ...
  override async forward(
    ...
    const detectionThreshold =
      options?.detectionThreshold ?? this.modelConfig.defaultDetectionThreshold ?? 0.7;
  ...
  static async fromCustomModel<L extends LabelEnum>(
    ...
    return new ObjectDetectionModule<L>(config.labelMap as ResolveLabels<L>, config, nativeModule);
  }
}

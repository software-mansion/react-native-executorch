...
} as const satisfies Record<ClassificationModelName, ClassificationConfig<LabelEnum>>;
 ...
 * @category Types
 */
export type ClassificationLabels<M extends ClassificationModelName> = ResolveLabelsFor<
  M,
  ModelConfigsType
>;
...
/** @internal */
type ResolveLabels<T extends ClassificationModelName | LabelEnum> = ResolveLabelsFor<
  T,
  ModelConfigsType
>;
...
> extends VisionLabeledModule<Record<keyof ResolveLabels<T>, number>, ResolveLabels<T>> {
  ...
  static async fromCustomModel<L extends LabelEnum>(
    ...
    return new ClassificationModule<L>(config.labelMap as ResolveLabels<L>, nativeModule);
  }

...
} as const satisfies Record<SemanticSegmentationModelName, SemanticSegmentationConfig<LabelEnum>>;
...
/** @internal */
type ResolveLabels<T extends SemanticSegmentationModelName | LabelEnum> = ResolveLabelsFor<
  T,
  ModelConfigsType
>;
...
> {
  ...
  static async fromModelName<C extends SemanticSegmentationModelSources>(
    ...
    const { preprocessorConfig } = ModelConfigs[modelName] as SemanticSegmentationConfig<LabelEnum>;
  ...
  static async fromCustomModel<L extends LabelEnum>(
    ...
    return new SemanticSegmentationModule<L>(config.labelMap as ResolveLabels<L>, nativeModule);
  }

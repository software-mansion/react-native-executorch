import { ResourceSource, LabelEnum, PixelData, Frame } from '../../types/common';
import {
 ...
 * @category Types
 */
export type InstanceSegmentationLabels<M extends InstanceSegmentationModelName> = ResolveLabels<M>;
 ...
 * @internal
 */
type ResolveLabels<T extends InstanceSegmentationModelName | LabelEnum> = ResolveLabelsFor<
  T,
  ModelConfigsType
>;
...
> extends VisionLabeledModule<SegmentedInstance<ResolveLabels<T>>[], ResolveLabels<T>> {
  ...
  override get runOnFrame(): (
    ...
    const defaultConfidenceThreshold = this.modelConfig.defaultConfidenceThreshold ?? 0.5;
    ...
    return (
      ...
      const returnMaskAtOriginalResolution = options?.returnMaskAtOriginalResolution ?? true;
  ...
  async forward(
    ...
    const confidenceThreshold =
      options?.confidenceThreshold ?? this.modelConfig.defaultConfidenceThreshold ?? 0.5;
    const iouThreshold = options?.iouThreshold ?? this.modelConfig.defaultIouThreshold ?? 0.5;
    const maxInstances = options?.maxInstances ?? 100;
    const returnMaskAtOriginalResolution = options?.returnMaskAtOriginalResolution ?? true;
    ...
    return nativeResult.map((inst) => ({
      ...
      maskWidth: inst.maskWidth,
      maskHeight: inst.maskHeight,
      label: (this.classIndexToLabel.get(inst.classIndex - this.labelEnumOffset) ??
        String(inst.classIndex)) as keyof ResolveLabels<T>,
      score: inst.score,
    }));
  }

import { Frame, LabelEnum, PixelData, ResourceSource } from '../../types/common';
import {
  Keypoint,
  PersonKeypoints,
  PoseDetections,
...
} as const satisfies Record<PoseEstimationModelName, PoseEstimationConfig<LabelEnum>>;
...
/** @internal */
type ResolveKeypoints<T extends PoseEstimationModelName | LabelEnum> = ResolveConfigOrType<
  T,
  ModelConfigsType,
  'keypointMap'
>;
...
> extends VisionModule<PoseDetections<ResolveKeypoints<T>>> {
  ...
  static async fromModelName<C extends PoseEstimationModelSources>(
    ...
    const modelConfig = ModelConfigs[namedSources.modelName] as PoseEstimationConfig<LabelEnum>;
    ...
    const nativeModule = await global.loadPoseEstimation(modelPath, normMean, normStd);
  ...
  static async fromCustomModel<K extends LabelEnum>(
    ...
    return new PoseEstimationModule<K>(keypointMap as ResolveKeypoints<K>, config, nativeModule);
  }
  ...
  override get runOnFrame(): (
    ...
    const defaultDetectionThreshold = this.modelConfig.defaultDetectionThreshold ?? 0.5;
    const defaultKeypointThreshold = this.modelConfig.defaultKeypointThreshold ?? 0.5;
    ...
    return (
      ...
      const keypointThreshold = options?.keypointThreshold ?? defaultKeypointThreshold;
      ...
      try {
        ...
        return mapPersonKeypoints<ResolveKeypoints<T>>(raw, keypointEntries, maxKeypointIndex);
      } finally {
  ...
  override async forward(
    ...
    const detectionThreshold =
      options?.detectionThreshold ?? this.modelConfig.defaultDetectionThreshold ?? 0.5;
    const keypointThreshold =
      options?.keypointThreshold ?? this.modelConfig.defaultKeypointThreshold ?? 0.5;

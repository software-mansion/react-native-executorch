...
type CellConfig<T> = T extends { base?: infer D; quant?: infer Q }
  ...
  : never;
type ConfigOf<V> = Extract<{ [K in keyof V]: CellConfig<V[K]> }[keyof V], { modelName: string }>;
...
function applySimulatorPolicy(
  ...
  throw new RnExecutorchError(
    ...
    'The MLX backend requires a physical iOS device and cannot run on the ' +
      'simulator.' +
      (variants.xnnpack
        ? " Pass `{ backend: 'xnnpack' }` (or omit `backend`) to run on the " + 'simulator.'
...
function selectBackend(
  ...
  if (platformDefaults) {
    if (Platform.OS === 'ios' && platformDefaults.ios) return platformDefaults.ios;
    if (Platform.OS === 'android' && platformDefaults.android) return platformDefaults.android;
...
function variant<const V extends AnyVariantMap>(
  ...
  type C = ConfigOf<V>;
  type B = BackendsOf<V>;
  return (opts: ModelOpts<B> = {}) => resolveVariant(variants, platformDefaults, opts) as C;
}
...
function styleTransferVariants<const Display extends string, const Slug extends string>(
  display: Display,
  slug: Slug
) {
...
const STYLE_TRANSFER_MOSAIC_VARIANTS = styleTransferVariants('mosaic', 'mosaic');
...
export const models = {
  ...
  semantic_segmentation: {
    deeplab_v3_resnet50: pair(M.DEEPLAB_V3_RESNET50, M.DEEPLAB_V3_RESNET50_QUANTIZED),
    deeplab_v3_resnet101: pair(M.DEEPLAB_V3_RESNET101, M.DEEPLAB_V3_RESNET101_QUANTIZED),
  ...
  text_embedding: {
    ...
    paraphrase_multilingual_minilm_l12_v2: base(M.PARAPHRASE_MULTILINGUAL_MINILM_L12_V2_QUANTIZED),

 ...
 */
export const getStructuredOutputPrompt = <T extends zCore.$ZodType<any, any, any>>(
  responseSchema: T | Schema
) => {
  const schemaObject: Schema | zCore.JSONSchema.JSONSchema =
    responseSchema instanceof zCore.$ZodType
      ? filterObjectKeys(z.toJSONSchema(responseSchema), ['$schema', 'additionalProperties'])
      : responseSchema;
 ...
 */
export const fixAndValidateStructuredOutput = <T extends zCore.$ZodType<any, any, any>>(
  output: string,
  responseSchema: T | Schema
): zCore.output<T> => {

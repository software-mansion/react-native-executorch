...
- **Do NOT throw raw `jsi::JSError`, `std::runtime_error`, or `std::invalid_argument`:** They reach JavaScript with no error code. Throw `CodedError` and register through `error::guarded(...)`. See the [Error Handling Skill](../error-handling/SKILL.md).
...
- Raise every failure as a `CodedError` and wrap the registration in `error::guarded(...)`, so the error reaches JavaScript with a `code`. See the [Error Handling Skill](../error-handling/SKILL.md).
...
namespace rnexecutorch::extensions::<domain>
{
    ...
    void install_customOp(jsi::Runtime &rt, jsi::Object &module)
    {
        ...
        // error::guarded turns any CodedError raised in the native stack into a
...
- [ ] Every failure is raised as `CodedError(ErrorCode::X, ...)`; no raw `jsi::JSError` / `std::runtime_error` / `std::invalid_argument` was introduced.
